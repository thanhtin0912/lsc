...
class Projects extends MX_Controller {
	...
	public function admincp_update($id=0){
		$this->load->model('projects_cata/projects_cata_model');
		if($id==0){
		...
		if($_POST){
			...
			if($_FILES){
				foreach($_FILES['images']['name'] as $k=>$v){
					if(isset($_FILES['images']['error'][$k]) && $_FILES['images']['error'][$k]!=4){
						$typeFileImage = strtolower(substr($_FILES['images']['type'][$k],0,5));
						// var_dump($typeFileImage);exit();
						if($typeFileImage == 'image'){
							$tmp_name[$k] = $_FILES['images']["tmp_name"][$k];
							$file_name[$k] = $_FILES['images']['name'][$k];
							$ext = strtolower(substr($file_name[$k], -4, 4));
							$images[$k] = date('Y').'/'.date('m').'/'.md5(time().'_'.SEO(substr($file_name[$k],0,-4))).$ext;
							$imagesName[] = $images[$k];
						}else{
			...
			if($this->model->saveManagement($imagesName,$fileName)){
				//Upload Image
				if($_FILES){
					$upload_path = BASEFOLDER.DIR_UPLOAD_PROJECTS;
					...
					foreach($imagesName as $k=>$v){
						if(isset($_FILES['images']['error'][$k]) && $_FILES['images']['error'][$k]!=4){
							move_uploaded_file($tmp_name[$k], $upload_path.$imagesName[$k]);
						}
					}
				}
				//End Upload Image
