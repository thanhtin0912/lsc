	...
	public function admincp_index(){
		...
		foreach ($products as $key => $p) {
			...
			foreach ($stores as $key => $s) {
				$inventoryProductStore = $this->model->getInventory($s->id, $p->id);
				$storeid = $s->id;
		...
		foreach ($products as $key => $p) {
			...
			foreach ($stores as $key => $s) {
				...
				$quoteProductStore = $this->model->getQuote($s->id, $p->id);
				...
				$storeid = $s->id;
				$object->$storeid = $sumEstimates ;
				$sum = $sum + $sumEstimates;
			}

			$object->estimates = $sum;
		...
		$data = array(
			...
			'compare'=> $compareInventoryStore,
			'estimatesOrder'=> $compareEstimatesOrderStore,
			'stores' => $stores
		);
