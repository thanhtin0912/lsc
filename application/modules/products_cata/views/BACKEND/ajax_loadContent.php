		...
		<table class="table table-striped table-bordered table-hover dataTable no-footer">
			<thead>
				<tr role="row">
					...
					<th class="sorting" onclick="sort('name_vn')" id="name">Hình ảnh</th>
					<th class="sorting" onclick="sort('name_en')" id="name">Name_vn</th>
					<th class="sorting" onclick="sort('type')" id="name">Danh mục cha</th>
			...
			<tbody>
				...
				<tr class="item_row<?=$i?> gradeX <?php ($k%2==0) ? print 'odd' : print 'even' ?>" role="row">
					...
					<td><a href="<?=PATH_URL_ADMIN.$module.'/update/'.$v->id?>"><img src="<?=resizeImage(PATH_URL.DIR_UPLOAD_CATA.$v->avata,100, 100)?>" /></a></td>
					<td><a href="<?=PATH_URL_ADMIN.$module.'/update/'.$v->id?>"><?= ($v->name_vn); ?></a></td>
					<td><a href="<?=PATH_URL_ADMIN.$module.'/update/'.$v->id?>"><?= cutText($v->cate_name, 20); ?></a></td>
