...
class Report_nhap_xuat_trong_ngay extends MX_Controller {

	private $module = 'report_nhap_xuat_trong_ngay';
	...
	public function admincp_ajaxLoadContent(){
		$products = $this->model->getProducts($_POST['store'], $_POST['name']);
		if($_POST['date']) {
			$date = $_POST['date'];
		} else {
		...
		if ($products) {
			...
			foreach ($products as $key => $p) {
				$imported = $this->model->totalImportDate($p->id, $storeId, $date);
				$exported = $this->model->totalExportdate($p->id, $storeId, $date);
				if ($imported || $exported) {
					$object = new StdClass;
					$object->import = serialize($imported);
					$object->export = serialize($exported);
					$object->name = $p->name;
					$object->totalImport = $this->model->totalImportToday($p->id, $storeId, $date)[0]->adjQty;
					$object->totalExport = $this->model->totalExportToday($p->id, $storeId, $date)[0]->adjQty;
					$historyDate[] = $object;
				}
			}
